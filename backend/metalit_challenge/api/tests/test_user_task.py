...
class UserTaskTestCaseCompleted(APITestCase):
  def setUp(self):
    """
    Setup before the test run
    """
    ...
    self.user_task = UserTask.objects.create(task=self.task,
                                            user=self.user,
                                            status="completed",
                                            completed_at="2022-01-08T14:05:36.166748+07:00")
    self.user_task_url =  reverse('user-task-completed', kwargs={'challenge_id':self.challenge.id})
    self.auth_url = reverse('generate-token', kwargs={'user_id':self.user.id})
...
class UserTaskTestCaseUncompleted(APITestCase):
  def setUp(self):
    """
    Setup before the test run
    """
    ...
    self.user_task = UserTask.objects.create(task=self.task,
                                            user=self.user,
                                            status="uncompleted")
    self.user_task_url =  reverse('user-task-uncompleted', kwargs={'challenge_id':self.challenge.id})
    self.auth_url = reverse('generate-token', kwargs={'user_id':self.user.id})
