...
from .views import (
    ...
    CreateUserTaskView,
    GenerateJWTMockup,
    CreateUserChallengeView,
    TaskVerificationView,
    TaskView,
    TestJWTResponse,
    UnverifiedTaskVerificationView,
    UserChallengeIndividualView,
    UserChallengeListView,
    UserChallengeTaskView,
    UserTaskListCompletedView,
    UserTaskListUncompletedView,
    UserTaskListView,
    VerifiedTaskVerificationView,
)
...
urlpatterns = [
    ...
    path(
        "user-challenge/challenge/<int:challenge_id>",
        UserChallengeIndividualView.as_view(),
    ),  # Auth, Perm
    ...
    path(
        "user-task/challenge/<int:challenge_id>/completed",
        UserTaskListCompletedView.as_view(),
    ),  # Auth
    path(
        "user-task/challenge/<int:challenge_id>/uncompleted",
        UserTaskListUncompletedView.as_view(),
    ),  # Auth
    path("user-task/create", CreateUserTaskView.as_view()),
    ### Endpoint related to user challenge and task ###
    path(
        "user-challenge-task/challenge/<int:challenge_id>/<int:user_id>",
        UserChallengeTaskView.as_view(),
    ),
]
